...
public class StartShapeDef
        implements SWSvgShapeDef<Start> {

    @Override
    public FontHandler<Start, SVGShapeView> newFontHandler() {
    ...
    public BiConsumer<Start, SVGShapeView> viewHandler() {
    ...
    public SVGShapeView<?> newViewInstance(final SWSVGViewFactory factory,
                                           final Start start) {
        return factory.startState().build(false);
    }

    @Override
    public Glyph getGlyph(final Class<? extends Start> type,
                          final String defId) {
        return SWGlyphFactory.START;
    }
}
